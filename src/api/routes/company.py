...
async def get_all_employees(department: str = Query(None, description="Filter by department")):
    """
...
async def get_projects(status: str = Query(None, description="Filter by status (active, completed, planning)")):
    """
...
async def get_skill_experts(skill_name: str):
    """
...
async def get_employee_projects(email: str):
    """
...
async def get_project_team(project_id: str):
    ...
    try:
        ...
        team = [
            ...
            for row in result if row["employee_name"]
        ]
